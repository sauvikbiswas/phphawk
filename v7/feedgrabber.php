...
function fetch_tags(){
    /* cURL parse*/
    $ch = curl_init("http://ws.audioscrobbler.com/2.0/?method=artist.gettoptags&artist=dark+project&api_key=f9d7d49161418480253e675889af7080");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_HEADER, 0);
    $xml = curl_exec($ch);
    curl_close($ch);
    ...
    $i = 0; 
    foreach($assoc_arr as $key => $element){
         if (($element['tag']=='TAG')&&($element['type']=='open')) $flag = 1;
         if (($element['tag']=='TAG')&&($element['type']=='close')) {
              $flag = 0;
              $i++;
         }
	if($element['tag']=='NAME') $items[$i]['name'] = $element['value'];
	if($element['tag']=='URL') $items[$i]['url'] = $element['value'];
	if($element['tag']=='COUNT') $items[$i]['count'] = $element['value'];
    } 
    return $items;
} 
...
function fetch_blogfeed($blog,$type){
	...
	if ($type=='facebook') $blogurl = 'https://www.facebook.com/feeds/page.php?id='.$blog.'&format=rss20';
...
function fetch_facebook_wall($id, $type){
    /* cURL parse*/
	if ($type == 'user') $fburl = 'https://www.facebook.com/feeds/notifications.php?id='.$id.'&viewer=545507802&key=AWiP2SBK06f0t89d&format=rss20';
	else $fburl = 'https://www.facebook.com/feeds/page.php?id='.$id.'&format=rss20';
	$curl = curl_init();
	...
	if ($type == 'user') $xml_data = str_replace('your', 'Dark Project\'s', $raw_xml);
    else $xml_data = $raw_xml;
